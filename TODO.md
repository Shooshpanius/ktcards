...
### ~~9. HTTP без HTTPS — нет TLS и перенаправления~~ ↩️ ОТМЕНЕНО (HTTPS на уровне контейнеров)
- **Файл:** `ktcards.client/nginx.conf`
...
- **Статус:** Ранее был добавлен HTTPS-блок, но HTTPS уже полностью реализован на уровне внешних контейнеров/реверс-прокси. nginx внутри контейнера возвращён к HTTP (порт 80). HSTS-заголовок убран из nginx.conf.
...
| 9 | HTTP без HTTPS / нет TLS | 🟡 Средняя | Средняя | ↩️ (HTTPS вне контейнера) |
