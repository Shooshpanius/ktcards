...
### 1. Предсказуемый токен аутентификации (GUID)
...
### 2. Токен хранится в `sessionStorage` (доступен через XSS)
...
### 3. Загрузка файла без проверки типа и размера (RCE / хранилище)
...
### 4. Path Traversal при удалении логотипа
...
### 5. Деструктивная операция `EnsureDeleted()` в production
- **Файл:** `ktcards.Server/Program.cs`
- **Проблема:** При ошибке миграции вызывается `db.Database.EnsureDeleted()` — **полное уничтожение базы данных в production!**
...
### 7. Нет эндпоинта выхода (logout) / инвалидации токена
- **Файл:** `ktcards.Server/Controllers/AuthController.cs`
